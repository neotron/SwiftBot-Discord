...
** abbreviations.
*/
static int compare_long(const char *user_option, const char *aov_long, int len) {
    unsigned char *uc, *ac;
    int diff = 0;
    int saved_len = len;
...
static int petopt_parse_option(PETOPT *pop, PETOPTS **pov, const char **optarg) {
    int i;
    ...
    *optarg = NULL;
    ...
    if(pop->ci == 0) {
        /* Non option or "-" */
        if(pop->argv[pop->ai][0] != '-' || (pop->argv[pop->ai][0] == '-' && pop->argv[pop->ai][1] == '\0')) {
        ...
        if(pop->argv[pop->ai][1] == '-') {
            ...
            for(i = 0; pop->pov[i].s != -1; i++) {
                if(pop->pov[i].l != NULL && compare_long(opt, pop->pov[i].l, len) == 0) {
                    /* Match found */
                    if(ix < 0) {
                        ix = i;
                    } else {
                ...
                *pov = &pop->pov[ix];
                ...
                if(pop->pov[ix].f) {
                    ...
                    if(boolval) {
                        ...
                        if(arg || (pop->pov[ix].f & POF_TYPEMASK) != POF_BOOL) {
                            return POE_INVALID;
                        }
                    ...
                    if(pop->ai < pop->argc && (pop->argv[pop->ai][0] != '-' || pop->argv[pop->ai][0] == '\0')) {
                        /* Have an argument, empty string or "-" */
                        if(((pop->pov[ix].f & POF_TYPEMASK) == POF_STR) || isdigit((int) pop->argv[pop->ai][0])) {
                    ...
                    if(*optarg == NULL && !((pop->pov[ix].f & POF_OPT) || ((pop->pov[ix]
                            .f & POF_TYPEMASK) == POF_BOOL))) {
                        pop->ai--;
                        return POE_MISSING;
                    }
                }
    ...
    *pov = &pop->pov[i];
    ...
    if(pop->pov[i].f) {
        if(pop->ai < pop->argc && (pop->ci != 0 || ((pop->argv[pop->ai][0] != '-') || pop->argv[pop->ai][1] == '\0'))) {
            if(((pop->pov[i].f & POF_TYPEMASK) == POF_STR) || (isdigit((int) pop->argv[pop->ai][pop->ci]) || pop
                    ->argv[pop->ai][pop->ci] == '-')) {
    ...
    /* Argument missing, and not optional? */
    if(pop->pov[i].f && !((pop->pov[i].f & POF_OPT) || (pop->pov[i].f & POF_TYPEMASK) == POF_BOOL) && *optarg == NULL) {
        if(pop->ci == 0) {
            pop->ai--;
...
int petopt_print_error(PETOPT *pop, PETOPTS *pov, int err, FILE *fp) {
    char buf[3];
    const char *arg;
...
int petopt_parse(PETOPT *pop, int *o_argc, char ***o_argv) {
    int err;
    const char *arg;
    PETOPTS *pov;
    ...
    while((err = petopt_parse_option(pop, &pov, &arg)) == 0) {
        if(pop->parse && (err = pop->parse(pop, pov, arg)) < 0) {
            goto Fail;
        }
        ...
        switch(pov->f & POF_TYPEMASK) {
            ...
            case POF_BOOL:
                if(arg && *arg != '\0') {
                    if(strcasecmp(arg, "true") == 0 || strcasecmp(arg, "yes") == 0 || strcasecmp(arg, "on") == 0) {
                        *(int *) (pov->v) = 1;
                    } else if(strcasecmp(arg, "false") == 0 || strcasecmp(arg, "no") == 0 || strcasecmp(arg,
                                                                                                        "off") == 0) {
                        *(int *) (pov->v) = 0;
                    } else {
                        err = POE_INVALID;
    ...
    Fail:
    if(err) {
        err = pop->errpr(pop, pov, err, (pop->f & POF_NOERRPRINT) ? NULL : stderr);
    }
...
int petopt_print_usage(PETOPT *pop, FILE *fp) {
    int i, len;
...
int petopt_setup(PETOPT **popp, int f, int argc, char **argv, PETOPTS *pov,
